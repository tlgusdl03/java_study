package week_12;

import javax.swing.*;
import java.awt.*;

public class ListEx extends JFrame {
    private String[] fruits = {"apple", "banana", "kiwi", "mango", "pear", "peach", "berry", "strawberry", "blackberry"};

    private ImageIcon[] images = {
            new ImageIcon("src/week_12/images/icon1.png"),
            new ImageIcon("src/week_12/images/icon2.png"),
            new ImageIcon("src/week_12/images/icon3.png"),
            new ImageIcon("src/week_12/images/icon4.png"),
    };

    public ListEx(){
        setTitle("리스트 만들기 예제");
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        Container c = getContentPane();
        c.setLayout(new FlowLayout());
        JList<String> strList = new JList<>(fruits);
        c.add(strList);

        JList<ImageIcon> imageList = new JList<>();
        imageList.setListData(images);
        c.add(imageList);

        JList<String> scrollLIst = new JList<>(fruits);
        c.add(new JScrollPane(scrollLIst));

        setSize(300, 300);
        setVisible(true);
    }

    public static void main(String[] args) {
        new ListEx();
    }
}
